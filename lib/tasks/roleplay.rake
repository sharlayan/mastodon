...
namespace :roleplay do
  ...
  task accept_all_notifications: :environment do
    ...
    Account.local.joins(:user).find_each do |account|
      ...
      ActiveRecord::Base.logger.silence { policy.save! }
      updated += 1
    end
  ...
  # DESTRUCTIVE: every matching post leaves the feeds, search and public pages.
  # 파괴적: 조건에 맞는 모든 게시물이 피드·검색·공개 페이지에서 사라집니다.
