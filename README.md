...
> [!WARNING]
> 본 포크 및 브랜치의 설치 비용을 받는 등의 행위를 금합니다.
> 호스트 서버 세팅 등 서비스 영역의 경우는 제외됩니다.
...
서버 내 계정 전환 기능은 보안을 위해 Misskey의 계정 전환 방식을 서버에 이관한 형태로 인증 정보를 주 계정에 종속시키는 방식 (구 tweetdeck team 형식)으로 서버에 전환 허용 플래그를 보존하도록 합니다. 연결된 계정의 경우 주 계정 연결을 직접 해제할 수 있습니다.

블루스카이 브릿지 게시물 전달 시 조용한 공개 게시물도 블루스카이 브릿지로 전송할 수 있도록 합니다.
...
연합 소프트웨어 확인 및 식별 (+Kmyblue), 서버 배지 표시 기능 등이 연합 정책에 포함됩니다.

## \* 인터페이스

Ref: F-Finene, Polyamspace, BirdSiteUI

게시글 접기 (Polyamspace, old glitch-soc) 및 읽지 않은 알림 게시글 접지 않음 (Polyamspace), 개인 멘션(DM)용 채팅형 인터페이스 및 읽은 DM 알림 자동 삭제 (리액션 포함 - 읽지 않은 DM 구분 가시성 강화) 기능을 지원합니다. 그려보세요 glitch-soc 기능 강화 (doodle), 덱 컬럼 조정 기능, local-setting 확장 등을 포함하고 있습니다.
...
작성 가능 글자수를 서버 코드 수정 및 재시작 없이 조정할 수 있습니다.
