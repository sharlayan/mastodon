...
SimpleNavigation::Configuration.run do |navigation|
  ...
  navigation.items do |n|
    ...
    n.item :admin, safe_join([material_symbol('manufacturing'), t('admin.title')]), nil, if: -> { current_user.can?(:view_dashboard, :manage_settings, :manage_rules, :manage_announcements, :manage_custom_emojis, :manage_webhooks, :manage_federation) && !self_destruct } do |s|
      ...
      s.item :board_announcements, safe_join([material_symbol('article'), t('admin.board_announcements.title')]), admin_board_announcements_path, highlights_on: %r{/admin/board_announcements}, if: -> { current_user.can?(:manage_announcements) }
      ...
      s.item :avatar_decorations, safe_join([material_symbol('circles'), t('admin.avatar_decorations.title')]), admin_avatar_decorations_path, highlights_on: %r{/admin/avatar_decoration}, if: -> { current_user.can?(:manage_custom_emojis) && Setting.avatar_decorations_enabled }
      ...
      s.item :relays, safe_join([material_symbol('captive_portal'), t('admin.relays.title')]), admin_relays_path, highlights_on: %r{/admin/relays}, if: -> { !limited_federation_mode? && !RoleplayModeHelper.roleplay_mode? && current_user.can?(:manage_federation) }
      s.item :drive_files, safe_join([material_symbol('cloud'), t('admin.drive_files.title')]), admin_drive_files_path, highlights_on: %r{/admin/drive/files}, if: -> { Setting.drive_enabled && current_user.can?(:manage_users) }
    end
