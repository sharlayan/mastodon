...
RSpec.describe CustomEmoji, :attachment_processing do
  ...
  describe 'Validations' do
    ...
    it { is_expected.to validate_length_of(:shortcode).is_at_least(1) }
    it { is_expected.to allow_values('cats', 'X').for(:shortcode) }
    it { is_expected.to_not allow_values('@#$@#$').for(:shortcode) }
  end
end
