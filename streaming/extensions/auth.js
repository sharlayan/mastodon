...
const permissionsSelect = `CASE
      WHEN COALESCE(user_roles.permissions, 0) & ${PERMISSION_ADMINISTRATOR} = ${PERMISSION_ADMINISTRATOR}
        THEN -1
      ELSE COALESCE(user_roles.permissions, 0) | COALESCE(everyone_role.permissions, 0)
    END AS permissions`;
...
const adminTimelineAccessSelect = isAdminTimelineEnabled()
  ? `, CASE
      ...
      ELSE COALESCE(user_roles.extra_permissions, 0)
        | COALESCE(everyone_role.extra_permissions, 0)
        | CASE
            WHEN (COALESCE(user_roles.extra_permissions, 0) | COALESCE(everyone_role.extra_permissions, 0)) & 2 = 2
              THEN 4
            ELSE 0
          END
  ...
  : '';
const ACCESS_TOKEN_QUERY = `SELECT oauth_access_tokens.id, oauth_access_tokens.resource_owner_id, users.account_id, users.chosen_languages, oauth_access_tokens.scopes, ${permissionsSelect}, COALESCE(oauth_applications.superapp, FALSE) AS superapp${adminTimelineAccessSelect} FROM oauth_access_tokens INNER JOIN users ON oauth_access_tokens.resource_owner_id = users.id INNER JOIN accounts ON accounts.id = users.account_id LEFT OUTER JOIN user_roles ON user_roles.id = users.role_id LEFT OUTER JOIN user_roles everyone_role ON everyone_role.id = -99 LEFT OUTER JOIN oauth_applications ON oauth_applications.id = oauth_access_tokens.application_id WHERE oauth_access_tokens.token = $1 AND oauth_access_tokens.revoked_at IS NULL AND (oauth_access_tokens.expires_in IS NULL OR oauth_access_tokens.created_at + oauth_access_tokens.expires_in * INTERVAL '1 second' > CURRENT_TIMESTAMP AT TIME ZONE 'UTC') AND users.disabled IS FALSE AND accounts.suspended_at IS NULL LIMIT 1`;
