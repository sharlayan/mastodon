...
class Scheduler::InstanceMetadataRefreshScheduler
  ...
  def perform
    ...
    missing_info_domains = InstanceMetadata.where('software IS NULL OR software = ? OR instance_name IS NULL OR instance_name = ?', '', '').limit(50).pluck(:domain)

    missing_info_domains.each do |domain|
      InstanceMetadataUpdateWorker.perform_async(domain)
    end

    outdated_domains = InstanceMetadata.where.not(software: [nil, '']).where.not(instance_name: [nil, '']).where('theme_color_updated_at IS NULL OR theme_color_updated_at < ?', 7.days.ago).limit(50).pluck(:domain)
